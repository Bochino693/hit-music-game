...
func _criar_hud_superior(tam_tela: Vector2) -> void:
	...
	## Centralizado, nao a direita: o rotulo ocupa o cartao inteiro, entao
	## alinhar a direita jogava "CRÉDITOS / 07" contra a borda e deixava o
	## cartao visivelmente torto.
	_hud_creditos = _criar_label_hud(0.031, COR_NEON_CIANO, HORIZONTAL_ALIGNMENT_CENTER)
...
## Numero de creditos ja pronto para caber no cartao.
##
## Dois digitos e o caso normal e mantem a largura estavel. Acima de 999 o
## texto vira "999+": o cartao tem largura fixa, entao um numero que cresce
## sem limite ou empurraria o layout ou seria encolhido pela fonte ate virar
## ilegivel. O saldo REAL nao e tocado aqui — isto e so a etiqueta.
func _texto_creditos(valor: int) -> String:
	if valor > 999:
		return "999+"
	return "%02d" % maxi(0, valor)
...
func _ao_apertar_start() -> void:
	...
	if modo_credito and creditos_maquina <= 0:
		...
		_atualizar_hud_operacao(true)
		return
	## Conta o START que REALMENTE abre partida. O aperto recusado por falta
	## de credito sai pelo `return` de cima e nao entra na conta — senao o
	## numero viraria "quantas vezes bateram no botao", nao quantas partidas
	## a maquina abriu, que e o que o operador precisa saber.
	ArcadeCounters.registrar_start(modo_credito)

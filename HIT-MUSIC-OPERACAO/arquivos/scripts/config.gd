...
var _counters_mode_label: Label
var _counters_body_label: Label
...
func _process(_delta: float) -> void:
	...
	if InputMap.has_action("input_select"):
		if not Input.is_action_pressed("input_select"):
			...
			## Mesma rota do F9. Antes o SELECT fechava o painel inteiro
			## mesmo estando DENTRO do teste, contrariando o "SELECT / F9:
			## VOLTAR" escrito na propria tela.
			_handle_back()
			return
	...
	if _in_counters_view:
		## So leitura: os botoes fisicos ficam inertes para nao andarem com
		## o cursor do menu que esta atras desta tela.
		return
...
func _refresh_menu_texts() -> void:
	...
	_menu_labels[MenuItem.COUNTERS].text = "CONTADOR DE PARTIDAS: %d" % (
		int(ArcadeCounters.contagem_atual()["total"])
	)
...
func _update_test_view() -> void:
	...
	var credito_pressed: bool = (
		InputMap.has_action("input_credito")
		and Input.is_action_pressed("input_credito")
	)
	if credito_pressed != _credito_pressed_state:
		_credito_pressed_state = credito_pressed
		_set_credito_box_active(credito_pressed)
...
# ---------------------------------------------------------------
## Mostra os dois baldes lado a lado: o que rodou na bancada e o que rodou
## no cliente. Ver so o balde de agora esconderia metade da historia da
## maquina — e justamente a comparacao entre os dois que responde "quanto
## disso fui eu testando".
func _build_counters_view(screen: Vector2) -> void:
	_counters_layer = CanvasLayer.new()
	_counters_layer.layer = 11
	...
	var topo := Panel.new()
	topo.position = Vector2(margin, margin)
	topo.size = Vector2(screen.x - margin * 2.0, top_height)
	topo.add_theme_stylebox_override("panel", _top_panel_style())
	_counters_layer.add_child(topo)

	var titulo: Label = _make_label(
		"CONTADOR DE PARTIDAS",
		int(top_height * 0.22),
		HORIZONTAL_ALIGNMENT_CENTER,
		font
	)
	titulo.position = Vector2(top_height * 0.13, top_height * 0.08)
	titulo.size = Vector2(topo.size.x - top_height * 0.26, top_height * 0.34)
	titulo.add_theme_color_override("font_color", Color(0.10, 0.85, 1.0, 1.0))
	topo.add_child(titulo)

	_counters_mode_label = _make_label(
		"",
		int(top_height * 0.12),
		HORIZONTAL_ALIGNMENT_CENTER,
		font
	)
	_counters_mode_label.position = Vector2(top_height * 0.13, top_height * 0.50)
	_counters_mode_label.size = Vector2(topo.size.x - top_height * 0.26, top_height * 0.26)
	topo.add_child(_counters_mode_label)

	var painel := Panel.new()
	painel.size = Vector2(_radius * 1.72, _radius * 1.10)
	painel.position = _center - painel.size * 0.5
	painel.add_theme_stylebox_override("panel", _panel_style())
	_counters_layer.add_child(painel)

	_counters_body_label = _make_label(
		"",
		int(_radius * 0.052),
		HORIZONTAL_ALIGNMENT_CENTER,
		font
	)
	_counters_body_label.position = Vector2(painel.size.x * 0.06, painel.size.y * 0.10)
	_counters_body_label.size = Vector2(painel.size.x * 0.88, painel.size.y * 0.80)
	_counters_body_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	painel.add_child(_counters_body_label)

	var dica: Label = _make_label(
		"SELECT / F9: VOLTAR",
		int(_radius * 0.027),
		HORIZONTAL_ALIGNMENT_CENTER,
		font
	)
	dica.position = Vector2(_center.x - _radius * 0.68, _center.y + _radius * 0.66)
	dica.size = Vector2(_radius * 1.36, _radius * 0.10)
	dica.add_theme_color_override("font_color", Color(0.60, 0.66, 0.78, 1.0))
	_counters_layer.add_child(dica)
...
func _refresh_counters_view() -> void:
	if _counters_body_label == null or not is_instance_valid(_counters_body_label):
		return
	var fabrica: Dictionary = ArcadeCounters.contagem("fabrica")
	var cliente: Dictionary = ArcadeCounters.contagem("cliente")
	var geral: Dictionary = ArcadeCounters.contagem_geral()
	var atual: String = ArcadeCounters.balde_atual()

	_counters_mode_label.text = (
		"CONTANDO AGORA EM: %s" % ("CLIENTE" if atual == "cliente" else "FÁBRICA / BANCADA")
	)
	_counters_mode_label.add_theme_color_override(
		"font_color",
		Color(1.0, 0.84, 0.24, 1.0) if atual == "cliente" else Color(0.55, 0.80, 1.0, 1.0)
	)

	_counters_body_label.text = "\n".join([
		"FÁBRICA / BANCADA",
		"TOTAL %d   •   LIVRE %d   •   CRÉDITO %d" % [
			int(fabrica["total"]), int(fabrica["livre"]), int(fabrica["credito"])
		],
		"",
		"CLIENTE",
		"TOTAL %d   •   LIVRE %d   •   CRÉDITO %d" % [
			int(cliente["total"]), int(cliente["livre"]), int(cliente["credito"])
		],
		"",
		"SOMA GERAL   %d" % int(geral["total"]),
	])
...
func _build_test_view(screen: Vector2) -> void:
	...
	## CREDITO, nao START.
	##
	## O teste existe pra conferir a fiacao do gabinete, e o botao de credito
	## e o unico que o operador nao consegue verificar jogando: START ele
	## prova em qualquer partida, credito so daria pra provar gastando ficha.
	var start_label: Label = _make_label(
		"CRÉDITO",
		int(_credito_box.size.y * 0.28),
		HORIZONTAL_ALIGNMENT_CENTER,
		font
	)
	start_label.size = _credito_box.size
	_credito_box.add_child(start_label)

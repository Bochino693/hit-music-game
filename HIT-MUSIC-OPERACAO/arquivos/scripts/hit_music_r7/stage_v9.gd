...
## A partida so esta DE FATO em andamento depois que a dificuldade foi
## confirmada. Antes disso a cena da musica ainda e uma tela de escolha:
## musica parada, mesa parada, e abrir o painel do operador ali nao
## atrapalha nada. Quem pergunta e o ArcadeShell, para decidir se libera o
## F9 — sem este metodo ele teria que tratar a cena inteira como partida e
## a tela de dificuldade perderia o atalho que ja funcionava.
func em_partida_ativa() -> bool:
	return not (_state == GameState.PRESENTATION and not _difficulty_confirmed)


func _input(event: InputEvent) -> void:
	if _state == GameState.PRESENTATION and not _difficulty_confirmed:
		# F9 aqui e responsabilidade do ArcadeShell (ver em_partida_ativa()).

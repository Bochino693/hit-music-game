...
const PREVIEW_DURATION: float = 15.0
...
var _preview_finished: bool = false
...
func _process(delta: float) -> void:
	# O gabinete nao deve exibir o ponteiro nem depois de Alt+Tab, mudanca de
	# foco ou qualquer outra cena/configuracao tentar torna-lo visivel.
	_hide_mouse_cursor()
	_visual_time += delta
...
func _hide_mouse_cursor() -> void:
	if Input.get_mouse_mode() != Input.MOUSE_MODE_HIDDEN:
		Input.set_mouse_mode(Input.MOUSE_MODE_HIDDEN)


func _update_preview(delta: float) -> void:
	if _preview_finished:
		return

	if _preview_started:
		_preview_elapsed += delta
		if _preview_elapsed >= PREVIEW_DURATION:
			_stop_preview()
		return
	...
	# Marca como iniciada mesmo quando a faixa nao possui midia valida. Assim,
	# uma falha de arquivo nao provoca uma tentativa de play a cada frame.
...
func _stop_preview() -> void:
	_preview_elapsed = PREVIEW_DURATION
	_preview_started = false
	_preview_finished = true
	if _video != null and _video.is_playing():
		_video.stop()
	if _preview_audio != null and _preview_audio.playing:
		_preview_audio.stop()

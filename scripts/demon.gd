extends Node2D
## ============================================================
## PLAY — HIT MUSIC — GURENGE / DEMON SLAYER (mesa redonda / tela circular)
##
...
## 2. CONTAGEM -- vídeo demon.ogv com contagem regressiva por cima.
## 3. JOGANDO -- vídeo demon.ogv centralizado no círculo, com
##    os 8 pontos, HUD, música demon e beatmap musical aleatório/adaptativo.
##
## Dentro de JOGANDO:
...
## - CAMINHO_MUSICA usa "res://songs/demon.mp3".
##   (ajuste a extensão/nome se o arquivo de áudio real tiver outro nome).
## - CAMINHO_IMAGEM_FASE usa "res://images/demon.jpeg".
## - CAMINHO_VIDEO_CONTAGEM usa "res://medias/demon.ogv".
## - CAMINHO_VIDEO_FUNDO usa "res://medias/demon.ogv".
...
## - NOME_FASE está como "GURENGE — DEMON SLAYER".
## - BEATMAP_DEMON pode ficar vazio: o motor multibanda escuta a música
##   em tempo real e cria toques/arrastes conforme pulsação, energia e timbre.
## - Esta cena usa o motor musical dinâmico da fase Gurenge / Demon Slayer,
...
const CAMINHO_IMAGEM_FASE: String = "res://images/demon.jpeg"
const NOME_FASE: String = "GURENGE - DEMON SLAYER"
const DURACAO_APRESENTACAO_SEG: float = 4.0

# --- Contagem regressiva de início ---
const CAMINHO_VIDEO_CONTAGEM: String = "res://medias/demon.ogv"
...
const CAMINHO_VIDEO_FUNDO: String = "res://medias/demon.ogv"

# Fallback automático: se você escreveu a pasta como medias em vez de mmedias,
# o script também tenta encontrar o vídeo em res://medias/demon.ogv.
const CAMINHO_VIDEO_FALLBACK_1: String = "res://medias/demon.ogv"

# --- Música ---
const CAMINHO_MUSICA: String = "res://songs/demon.mp3"
...
const PERFIL_JOGABILIDADE: Dictionary = {
	# Gurenge usa um perfil de anime rock intenso: bumbo, caixa, guitarra e ataques
	# vocais geram toques rápidos. Arrastes aparecem em viradas, sustentação melódica
	# e entradas fortes de refrão, mantendo a fase energética sem virar poluição.
	"nome": "GURENGE_ROCK_INTENSO",
	"sensibilidade": 1.12,
	"densidade": 0.96,
	"preferencia_arraste": 0.25,
	"complexidade_arraste": 0.68,
	"distancia_saltos": 0.92,
	"janela_toque_extra": 0.018,
}

# --- Cores neon (Gurenge / Demon Slayer: vermelho nichirin, branco e dourado) ---
const COR_NEON_VERDE: Color = Color(1.0, 0.035, 0.085)
const COR_NEON_CIANO: Color = Color(1.0, 0.96, 0.96)
const COR_ACERTO: Color = Color(1.0, 0.78, 0.16)
const COR_ERRO: Color = Color(0.62, 0.0, 0.035)
...
## Se BEATMAP_DEMON estiver vazio, o jogo não usa mais fases fixas por
...
# Ajuste vertical exclusivo da fase Gurenge — Demon Slayer.
...
# Identidade visual Demon Slayer: vermelho nichirin, branco e preto.
const COR_DEMON_VERMELHO: Color = Color(0.98, 0.018, 0.055, 1.0)
const COR_DEMON_BRANCO: Color = Color(1.0, 0.985, 0.985, 1.0)
...
# Proporção real do vídeo (largura / altura) -- AJUSTE pra bater com o demon.ogv
...
const CAMINHO_IMAGEM_MODAL: String = "res://images/demon.jpeg"
...
func _ready() -> void:
	...
	_beatmap = BEATMAP_DEMON.duplicate(true)
	if not _beatmap.is_empty():
		push_warning(
			"Usando o BEATMAP_DEMON escrito à mão -- a detecção de batida em tempo real fica desligada nesta partida."
		)
		...
		push_warning(
			"BEATMAP_DEMON vazio -- usando detecção de batida em tempo real (grave/bumbo) da música demon."
		)
	else:
...
func _desenhar_onda_borda_musical() -> void:
	...
	# Identidade Demon: carmim nos graves, azul-mar nos médios e ouro/branco
...
func _criar_video_generico(caminho: String, loop: bool) -> void:
	...
	# O áudio oficial do jogo vem do AudioStreamPlayer usando demon.mp3.
...
# ---------------------------------------------------------------
# 1) APRESENTAÇÃO -- demon.jpeg + nome da fase
# ---------------------------------------------------------------
...
void fragment() {
	...
	// Duas bordas Demon Slayer: vermelho externo e branco interno.
...
func _criar_hud() -> void:
	...
	estilo.border_color = Color(COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.86)
	estilo.set_corner_radius_all(int(altura_painel * 0.5))
	estilo.shadow_color = Color(COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.42)
	...
	config_pontuacao.outline_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.86
	)
	...
	estilo_tempo.border_color = Color(COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.94)
	estilo_tempo.set_corner_radius_all(int(altura_tempo * 0.5))
	estilo_tempo.shadow_color = Color(COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.24)
	...
	config_tempo.outline_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.88
	)
...
func _mostrar_modal_continuar() -> void:
	...
	estilo.border_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.98
	)
	estilo.set_corner_radius_all(int(_raio_circulo_px * 0.050))
	estilo.shadow_color = Color(
		COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.24
	)
	estilo.shadow_size = 26
	_modal_painel.add_theme_stylebox_override("panel", estilo)
	_modal_layer.add_child(_modal_painel)

	# Fundo temático do resultado. Usa exatamente demon.jpeg e mantém a
	...
	estilo_moldura.border_color = Color(
		COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.74
	)
	...
	# Cabeçalho Demon em vidro vermelho.
	...
	estilo_faixa.bg_color = Color(
		COR_DEMON_VERMELHO.r * 0.24,
		COR_DEMON_VERMELHO.g * 0.24,
		COR_DEMON_VERMELHO.b * 0.24,
		0.82
	)
	estilo_faixa.set_border_width_all(2)
	estilo_faixa.border_color = Color(
		COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.82
	)
	estilo_faixa.set_corner_radius_all(int(altura * 0.055))
	estilo_faixa.shadow_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.42
	)
	...
	cfg_titulo.font_color = COR_DEMON_BRANCO
	...
	cfg_titulo.shadow_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.64
	)

	_modal_label_titulo = Label.new()
	_modal_label_titulo.label_settings = cfg_titulo
	_modal_label_titulo.text = "GURENGE - DEMON SLAYER"
	...
	cfg_score.font_color = COR_DEMON_BRANCO
	cfg_score.outline_size = 6
	cfg_score.outline_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.96
	)
	cfg_score.shadow_size = 18
	cfg_score.shadow_color = Color(
		COR_DEMON_VERMELHO.r, COR_DEMON_VERMELHO.g, COR_DEMON_VERMELHO.b, 0.52
	)
	...
	barra_cheia.shadow_color = Color(
		COR_DEMON_BRANCO.r, COR_DEMON_BRANCO.g, COR_DEMON_BRANCO.b, 0.34
	)
	...
	cfg_acao.font_color = COR_DEMON_BRANCO
...
func _registrar_acerto(pontos: int) -> void:
	...
	# rotinas de LED permanecem exatamente com o comportamento original do Demon.
...
# ---------------------------------------------------------------
# BEATMAP DE TESTE (usado só se BEATMAP_DEMON estiver vazio)
# ---------------------------------------------------------------


func _gerar_beatmap_demon_por_fases(duracao_seg: float) -> Array:
	var eventos: Array = []
	...
	#
	# A música real desta fase é demon.mp3.

extends Node2D
## ============================================================
## PLAY — HIT MUSIC — FAIRY TAIL ZERO / ASHITA WO NARASE (mesa redonda / tela circular)
##
...
## 2. CONTAGEM -- vídeo fairy.ogv com contagem regressiva por cima.
## 3. JOGANDO -- vídeo fairy.ogv centralizado no círculo, com
##    os 8 pontos, HUD, música fairy e beatmap musical aleatório/adaptativo.
##
## Dentro de JOGANDO:
...
## - CAMINHO_MUSICA usa "res://songs/fairy.mp3".
##   (ajuste a extensão/nome se o arquivo de áudio real tiver outro nome).
## - CAMINHO_IMAGEM_FASE usa "res://images/fairy.jpeg".
## - CAMINHO_VIDEO_CONTAGEM usa "res://medias/fairy.ogv".
## - CAMINHO_VIDEO_FUNDO usa "res://medias/fairy.ogv".
...
## - NOME_FASE está como "FAIRY TAIL ZERO - ASHITA WO NARASE".
## - BEATMAP_FAIRY pode ficar vazio: o motor multibanda escuta a música
##   em tempo real e cria toques/arrastes conforme pulsação, energia e timbre.
## - Esta cena usa o motor musical dinâmico da fase ASHITA WO NARASE / Fairy Tail Zero,
...
const CAMINHO_IMAGEM_FASE: String = "res://images/fairy.jpeg"
const NOME_FASE: String = "FAIRY TAIL ZERO - ASHITA WO NARASE"
const DURACAO_APRESENTACAO_SEG: float = 4.0

# --- Contagem regressiva de início ---
const CAMINHO_VIDEO_CONTAGEM: String = "res://medias/fairy.ogv"
...
const CAMINHO_VIDEO_FUNDO: String = "res://medias/fairy.ogv"

# Fallback automático: se você escreveu a pasta como medias em vez de mmedias,
# se o projeto antigo usava a pasta mmedias, o script também tenta res://mmedias/fairy.ogv.
const CAMINHO_VIDEO_FALLBACK_1: String = "res://mmedias/fairy.ogv"

# --- Música ---
const CAMINHO_MUSICA: String = "res://songs/fairy.mp3"
...
const PERFIL_JOGABILIDADE: Dictionary = {
	# Perfil Fairy: rock de anime acelerado, alternância fluida e arrastes controlados.
	# A densidade foi limitada para preservar leitura visual e estabilidade no gabinete.
	"nome": "FAIRY_UPBEAT_ANIME_ROCK",
	"sensibilidade": 1.04,
	"densidade": 0.82,
	"preferencia_arraste": 0.24,
	"complexidade_arraste": 0.60,
	"distancia_saltos": 0.74,
	"janela_toque_extra": 0.028,
}

# --- Cores neon (ASHITA WO NARASE / Fairy Tail Zero: azul, rosa, branco e dourado mágico) ---
const COR_FAIRY_AZUL_MAGICO: Color = Color(0.16, 0.62, 1.0, 1.0)  # azul mágico Fairy
const COR_FAIRY_ROSA_LUMINOSO: Color = Color(1.0, 0.24, 0.72, 1.0)  # rosa luminoso Fairy
const COR_ACERTO: Color = Color(1.0, 0.86, 0.22, 1.0)
const COR_ERRO: Color = Color(0.24, 0.02, 0.40, 1.0)
...
## Se BEATMAP_FAIRY estiver vazio, o jogo não usa mais fases fixas por
...
# Ajuste vertical exclusivo da fase Fairy Tail Zero — ASHITA WO NARASE.
...
# Identidade visual Fairy Tail Zero: azul mágico, rosa luminoso, branco e azul profundo.
const COR_FAIRY_AZUL: Color = Color(0.16, 0.62, 1.0, 1.0)
const COR_FAIRY_ROSA_CLARO: Color = Color(1.0, 0.72, 0.92, 1.0)
...
# --- Desempenho ---
# PERFIL DE DESEMPENHO FAIRY: visual limitado, lógica/input mantidos em 60 FPS.
...
const VISUAL_FPS_NORMAL: float = 40.0
const VISUAL_FPS_REDUZIDO: float = 24.0
...
const MAX_SISTEMAS_PARTICULAS_ATIVOS: int = 8
...
# Proporção real do vídeo (largura / altura) -- AJUSTE pra bater com o fairy.ogv
...
const CAMINHO_IMAGEM_MODAL: String = "res://images/fairy.jpeg"
...
const CORES_ORBE_POR_PONTO: Array = [
	Color(0.16, 0.62, 1.0, 1.0),   # A azul mágico
	Color(1.0, 0.24, 0.72, 1.0),   # B rosa luminoso
	Color(0.18, 0.92, 1.0, 1.0),   # C ciano
	Color(0.64, 0.28, 1.0, 1.0),   # D violeta
	Color(1.0, 0.82, 0.20, 1.0),   # E dourado
	Color(0.26, 1.0, 0.58, 1.0),   # F verde magia
	Color(0.96, 0.98, 1.0, 1.0),   # G branco azulado
	Color(1.0, 0.50, 0.16, 1.0),   # H laranja
]

const COR_TOQUE_ORBE: Color = Color(0.16, 0.62, 1.0, 1.0)
const COR_SLIDE_AMARELO: Color = Color(1.0, 0.72, 0.18, 1.0)
const COR_SLIDE_PRETO: Color = Color(0.008, 0.014, 0.045, 1.0)
...
func _ready() -> void:
	...
	_beatmap = BEATMAP_FAIRY.duplicate(true)
	if not _beatmap.is_empty():
		push_warning(
			"Usando o BEATMAP_FAIRY escrito à mão -- a detecção de batida em tempo real fica desligada nesta partida."
		)
		...
		push_warning(
			"BEATMAP_FAIRY vazio -- usando detecção de batida em tempo real (grave/bumbo) da música fairy."
		)
	else:
...
func _desenhar_area_dedicada_central() -> void:
	...
	draw_circle(
		...
		Color(COR_FAIRY_ROSA_LUMINOSO.r, COR_FAIRY_ROSA_LUMINOSO.g, COR_FAIRY_ROSA_LUMINOSO.b, 0.018 + pulso * 0.012)
	)
	...
	draw_arc(
		...
		giro + PI * 1.38,
		72,
		Color(COR_FAIRY_ROSA_LUMINOSO.r, COR_FAIRY_ROSA_LUMINOSO.g, COR_FAIRY_ROSA_LUMINOSO.b, 0.105 + pulso * 0.035),
		2.7,
		true
	)
	draw_arc(
		...
		-giro * 0.82 + PI * 1.72,
		72,
		Color(COR_FAIRY_AZUL_MAGICO.r, COR_FAIRY_AZUL_MAGICO.g, COR_FAIRY_AZUL_MAGICO.b, 0.092 + pulso * 0.032),
		2.8,
		true
	)
	draw_arc(
	...
	for i in range(16 if not _visual_reduzido else 10):
		...
		var cor: Color = COR_FAIRY_ROSA_LUMINOSO.lerp(
			COR_FAIRY_AZUL_MAGICO, 0.5 + 0.5 * sin(float(i) * 0.9 + tempo * 2.0)
		)
...
func _cor_caminho_evento(evento: Dictionary) -> Color:
	...
	if evento.is_empty():
		return COR_FAIRY_ROSA_LUMINOSO
...
func _desenhar_nucleo_central() -> void:
	...
	draw_circle(
		_centro_tela,
		raio_nucleo * 2.15,
		Color(COR_FAIRY_AZUL_MAGICO.r, COR_FAIRY_AZUL_MAGICO.g, COR_FAIRY_AZUL_MAGICO.b, 0.050 + pulso * 0.050)
	)
	draw_circle(
		_centro_tela,
		raio_nucleo * 1.35,
		Color(COR_FAIRY_ROSA_LUMINOSO.r, COR_FAIRY_ROSA_LUMINOSO.g, COR_FAIRY_ROSA_LUMINOSO.b, 0.035 + pulso * 0.035)
	)
	draw_arc(
		_centro_tela,
		raio_nucleo * 1.82,
	...
	draw_arc(
		...
		-giro + PI * 1.35,
		48,
		Color(COR_FAIRY_AZUL_MAGICO.r, COR_FAIRY_AZUL_MAGICO.g, COR_FAIRY_AZUL_MAGICO.b, 0.68 + pulso * 0.22),
		3.1,
		true
	)
	draw_arc(
		...
		giro * 1.45 + PI * 1.65,
		48,
		Color(COR_FAIRY_ROSA_LUMINOSO.r, COR_FAIRY_ROSA_LUMINOSO.g, COR_FAIRY_ROSA_LUMINOSO.b, 0.70 + pulso * 0.20),
		2.7,
		true
	)
...
func _desenhar_anel_volume() -> void:
	...
	if proporcao > 0.001:
		var cor_nivel: Color = COR_FAIRY_ROSA_LUMINOSO.lerp(COR_ERRO, 1.0 - proporcao)
		draw_arc(
			_centro_tela,
			raio_anel,
			inicio_ang,
...
func _registrar_ponto_rastro_arraste(pos: Vector2) -> void:
	...
	var cor: Color = COR_FAIRY_ROSA_LUMINOSO
...
func _desenhar_rastro_arraste() -> void:
	...
	if _arraste_gesto_ativo:
		var cor_cursor: Color = COR_FAIRY_ROSA_LUMINOSO
		if _arraste_evento_atual != null:
...
func _criar_video_generico(caminho: String, loop: bool) -> void:
	...
	# O áudio oficial do jogo vem do AudioStreamPlayer usando fairy.mp3.
...
# ---------------------------------------------------------------
# 1) APRESENTAÇÃO -- fairy.jpeg + nome da fase
# ---------------------------------------------------------------
func _iniciar_apresentacao_fase() -> void:
	...
	config_nome.shadow_color = Color(COR_FAIRY_AZUL_MAGICO.r, COR_FAIRY_AZUL_MAGICO.g, COR_FAIRY_AZUL_MAGICO.b, 0.6)
...
func _iniciar_contagem() -> void:
	...
	config_contagem.shadow_color = Color(COR_FAIRY_ROSA_LUMINOSO.r, COR_FAIRY_ROSA_LUMINOSO.g, COR_FAIRY_ROSA_LUMINOSO.b, 0.7)
...
void fragment() {
	...
	// Duas bordas Fairy: azul mágico externo e rosa luminoso interno.
...
func _criar_hud() -> void:
	...
	estilo.border_color = Color(COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.86)
	estilo.set_corner_radius_all(int(altura_painel * 0.5))
	estilo.shadow_color = Color(COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.42)
	...
	config_pontuacao.outline_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.86
	)
	...
	estilo_tempo.border_color = Color(COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.94)
	estilo_tempo.set_corner_radius_all(int(altura_tempo * 0.5))
	estilo_tempo.shadow_color = Color(COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.24)
	...
	config_tempo.outline_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.88
	)
	...
	config_combo.font_color = COR_FAIRY_AZUL_MAGICO
...
func _mostrar_modal_continuar() -> void:
	...
	estilo.border_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.98
	)
	estilo.set_corner_radius_all(int(_raio_circulo_px * 0.050))
	estilo.shadow_color = Color(
		COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.24
	)
	estilo.shadow_size = 26
	_modal_painel.add_theme_stylebox_override("panel", estilo)
	_modal_layer.add_child(_modal_painel)

	# Fundo temático do resultado. Usa exatamente fairy.jpeg e mantém a
	...
	estilo_moldura.border_color = Color(
		COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.74
	)
	...
	# Cabeçalho Fairy em vidro azul/rosa.
	...
	estilo_faixa.bg_color = Color(
		COR_FAIRY_AZUL.r * 0.24,
		COR_FAIRY_AZUL.g * 0.24,
		COR_FAIRY_AZUL.b * 0.24,
		0.82
	)
	estilo_faixa.set_border_width_all(2)
	estilo_faixa.border_color = Color(
		COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.82
	)
	estilo_faixa.set_corner_radius_all(int(altura * 0.055))
	estilo_faixa.shadow_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.42
	)
	...
	cfg_titulo.font_color = COR_FAIRY_ROSA_CLARO
	...
	cfg_titulo.shadow_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.64
	)
	...
	cfg_score.font_color = COR_FAIRY_ROSA_CLARO
	cfg_score.outline_size = 6
	cfg_score.outline_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.96
	)
	cfg_score.shadow_size = 18
	cfg_score.shadow_color = Color(
		COR_FAIRY_AZUL.r, COR_FAIRY_AZUL.g, COR_FAIRY_AZUL.b, 0.52
	)
	...
	barra_cheia.shadow_color = Color(
		COR_FAIRY_ROSA_CLARO.r, COR_FAIRY_ROSA_CLARO.g, COR_FAIRY_ROSA_CLARO.b, 0.34
	)
	...
	cfg_acao.font_color = COR_FAIRY_ROSA_CLARO
...
# ---------------------------------------------------------------
# BEATMAP DE TESTE (fallback; o motor dinâmico usa BEATMAP_FAIRY vazio)
# ---------------------------------------------------------------


func _gerar_beatmap_fairy_por_fases(duracao_seg: float) -> Array:
	var eventos: Array = []
	...
	#
	# A música real desta fase é fairy.mp3.

...
# Comunicação direta COM5 usando FileAccess.
# Mantive em 9600 porque o GDScript puro não configura baudrate; no Windows a COM costuma abrir em 9600.
# Se não abrir, a solução 100% robusta é Godot .NET/plugin serial, mas este arquivo fica só em GDScript.
const SERIAL_ATIVADO: bool = true
const SERIAL_PORT_PATHS: Array[String] = ["\\\\.\\COM5", "COM5"]
const SERIAL_WARMUP_MS: int = 1800
const SERIAL_RETRY_MS: int = 1200

var _led_assinatura_atual: String = ""
var _led_feedback_bloqueio_ate_msec: int = 0
var _serial_file: FileAccess = null
var _serial_port_path_aberta: String = ""
var _serial_pronto_em_msec: int = 0
var _serial_ultima_tentativa_msec: int = -999999
...
func _atualizar_leds_guia(tempo_musica: float) -> void:
	...
	for evento in _eventos_ativos:
		...
		var tempo_fim: float = tempo_inicio
		if evento["tipo"] == "arraste":
			tempo_fim = float(evento["tempo_fim"])
		if tempo_musica > tempo_fim + JANELA_ACERTO_SEG:
			continue
		var distancia: float = abs(tempo_inicio - tempo_musica)
	...
	if melhor_evento["tipo"] == "toque":
		var ponto: int = int(melhor_evento["ponto"])
		var cor: Color = _cor_orbe_por_ponto(ponto)
		_arduino_enviar_unico("LED %d %d %d %d" % [ponto, int(cor.r * 255.0), int(cor.g * 255.0), int(cor.b * 255.0)])
	elif melhor_evento["tipo"] == "arraste":
		var caminho: Array = melhor_evento["caminho"]
		if _arraste_evento_atual != null and _arraste_evento_atual == melhor_evento:
			_arduino_enviar_unico(_cmd_led_progresso(caminho, _arraste_indice_atual))
		else:
			_arduino_enviar_unico(_cmd_led_path(caminho))

func _cmd_led_path(caminho: Array) -> String:
	var cmd: String = "PATH " + str(caminho.size())
...
func _cmd_led_progresso(caminho: Array, passo_atual: int) -> String:
	var cmd: String = "PROG " + str(passo_atual) + " " + str(caminho.size())
	for indice in caminho:
		cmd += " " + str(int(indice))
	return cmd
...
func _cmd_led_slide_ok(caminho: Array) -> String:
	var cmd: String = "SLIDEOK " + str(caminho.size())
	for indice in caminho:
		cmd += " " + str(int(indice))
	return cmd
...
func _arduino_inicializar() -> void:
	if not SERIAL_ATIVADO:
		return
	if _serial_file != null:
		return

	var agora: int = Time.get_ticks_msec()
	if agora - _serial_ultima_tentativa_msec < SERIAL_RETRY_MS:
		return
	_serial_ultima_tentativa_msec = agora

	for caminho in SERIAL_PORT_PATHS:
		var porta := FileAccess.open(caminho, FileAccess.READ_WRITE)
		if porta != null:
			_serial_file = porta
			_serial_port_path_aberta = caminho
			_serial_pronto_em_msec = Time.get_ticks_msec() + SERIAL_WARMUP_MS
			_serial_aviso_mostrado = false
			print("HIT MUSIC - COM5 aberta direto pelo Godot em: ", caminho)
			return

	if not _serial_aviso_mostrado:
		_serial_aviso_mostrado = true
		push_warning("Não consegui abrir a COM5 direto pelo Godot. Feche Monitor Serial/Arduino IDE e confirme se a porta é COM5. GDScript puro não configura baudrate; o Arduino está em 9600.")
...
func _arduino_tick_serial() -> void:
	...
	if _serial_file == null:
		_arduino_inicializar()
		return
	if Time.get_ticks_msec() < _serial_pronto_em_msec:
		return
	if _serial_pendentes.is_empty():
		return
	var copia := _serial_pendentes.duplicate()
	_serial_pendentes.clear()
	for linha in copia:
		_arduino_escrever_serial(linha)
...
func _arduino_enviar_linha(linha: String) -> void:
	...
	# Arduino reinicia quando a COM abre. Segura os primeiros comandos para não perder BLINKALL/READY.
	if Time.get_ticks_msec() < _serial_pronto_em_msec:
		_serial_pendentes.append(linha)
		if _serial_pendentes.size() > 8:
...
func _arduino_escrever_serial(linha: String) -> void:
	if _serial_file == null:
		return
	var pacote: String = linha.strip_edges() + "\n"
	_serial_file.store_string(pacote)
	_serial_file.flush()
	# print("LED -> ", pacote.strip_edges())


func _notification(what: int) -> void:
	if what == NOTIFICATION_WM_CLOSE_REQUEST or what == NOTIFICATION_PREDELETE:
		if _serial_file != null:
			_serial_file.store_string("CLEAR\n")
			_serial_file.flush()
			_serial_file.close()
			_serial_file = null
...
func _processar_inputs_toque(tempo_musica: float) -> void:
	for i in range(NUM_PONTOS):
		if Input.is_action_just_pressed(INPUTS_PONTOS[i]):
			# Mapa direto: input_a -> D2, input_b -> D3 ... input_h -> D9.
			# Este comando é independente do beatmap e serve também para testar a fiação.
	...
	for evento in _eventos_ativos:
		if evento["tipo"] == "toque" and evento["ponto"] == indice_ponto and not evento.get("resolvido", false):
			if abs(tempo_musica - evento["tempo"]) <= JANELA_ACERTO_SEG:
				...
				_arduino_enviar_unico("OK " + str(indice_ponto))
				return
...
func _resolver_arraste_completo() -> void:
	...
	if not dentro_do_tempo:
		...
		_arduino_enviar_unico("ERR " + str(int(caminho[idx_falha])))
	else:
		evento["resolvido"] = true
		...
		_arduino_enviar_unico(_cmd_led_slide_ok(caminho))
...
func _registrar_erro(_evento: Dictionary) -> void:
	...
	if indice_led >= 0:
		_led_feedback_bloqueio_ate_msec = Time.get_ticks_msec() + 520
		_arduino_enviar_unico("ERR " + str(indice_led))

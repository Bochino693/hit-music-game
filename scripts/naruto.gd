extends Node2D
## ============================================================
## PLAY — HIT MUSIC — NARUTO SHIPPUDEN / BLUE BIRD (mesa redonda / tela circular)
##
...
## 2. CONTAGEM -- vídeo naruto.ogv com contagem regressiva por cima.
## 3. JOGANDO -- vídeo naruto.ogv centralizado no círculo, com
##    os 8 pontos, HUD, música Blue Bird e beatmap musical aleatório/adaptativo.
##
## Dentro de JOGANDO:
...
## - CAMINHO_MUSICA usa "res://songs/naruto.mp3".
##   (ajuste a extensão/nome se o arquivo de áudio real tiver outro nome).
## - CAMINHO_IMAGEM_FASE usa "res://images/naruto.jpeg".
## - CAMINHO_VIDEO_CONTAGEM usa "res://medias/naruto.ogv".
## - CAMINHO_VIDEO_FUNDO usa "res://medias/naruto.ogv".
...
## - NOME_FASE está como "NARUTO SHIPPUDEN - BLUE BIRD".
## - BEATMAP_BLUE_BIRD pode ficar vazio: o motor multibanda escuta a música
##   em tempo real e cria toques/arrastes conforme pulsação, energia e timbre.
## - Esta cena usa o motor musical dinâmico da fase BLUE BIRD / Naruto Shippuden,
...
const CAMINHO_IMAGEM_FASE: String = "res://images/naruto.jpeg"
const NOME_FASE: String = "NARUTO SHIPPUDEN - BLUE BIRD"
const TITULO_APRESENTACAO: String = "NARUTO SHIPPUDEN
BLUE BIRD"
const DURACAO_APRESENTACAO_SEG: float = 4.0

# --- Contagem regressiva de início ---
const CAMINHO_VIDEO_CONTAGEM: String = "res://medias/naruto.ogv"
...
const CAMINHO_VIDEO_FUNDO: String = "res://medias/naruto.ogv"

# Fallback automático: se você escreveu a pasta como medias em vez de mmedias,
# se o projeto antigo usava a pasta mmedias, o script também tenta res://mmedias/naruto.ogv.
const CAMINHO_VIDEO_FALLBACK_1: String = "res://medias/naturo.ogv"

# --- Música ---
const CAMINHO_MUSICA: String = "res://songs/naruto.mp3"
...
const PERFIL_JOGABILIDADE: Dictionary = {
	# Blue Bird tem andamento acelerado e refrão forte, mas a densidade permanece
	# limitada para preservar leitura visual, precisão do touch e estabilidade.
	"nome": "BLUE_BIRD_UPBEAT_JROCK",
	"sensibilidade": 1.03,
	"densidade": 0.80,
	"preferencia_arraste": 0.23,
	"complexidade_arraste": 0.58,
	"distancia_saltos": 0.76,
	"janela_toque_extra": 0.030,
}

# --- Cores neon (BLUE BIRD / Naruto Shippuden: azul, rosa, branco e dourado mágico) ---
const COR_BLUE_BIRD_AZUL_CEU: Color = Color(0.12, 0.64, 1.0, 1.0)  # azul do céu
const COR_BLUE_BIRD_LARANJA: Color = Color(1.0, 0.43, 0.035, 1.0)  # laranja Naruto
const COR_ACERTO: Color = Color(1.0, 0.88, 0.18, 1.0)
const COR_ERRO: Color = Color(0.38, 0.02, 0.10, 1.0)
...
## Se BEATMAP_BLUE_BIRD estiver vazio, o jogo não usa mais fases fixas por
...
const ANALISE_INTERVALO_SEG: float = 0.033
...
const ESCALA_ALCANCE_PARTICULAS: float = 0.68

# Ajuste vertical exclusivo da fase Naruto Shippuden — BLUE BIRD.
...
# Identidade visual Naruto Shippuden: azul mágico, rosa luminoso, branco e azul profundo.
const COR_BLUE_BIRD_AZUL: Color = Color(0.08, 0.44, 1.0, 1.0)
const COR_BLUE_BIRD_AZUL_CLARO: Color = Color(0.62, 0.88, 1.0, 1.0)
...
const ARRASTE_MAX_AMOSTRAS_MOVIMENTO: int = 14
...
const ARRASTE_RASTRO_MAX_PONTOS: int = 30
...
# --- Desempenho ---
# PERFIL DE DESEMPENHO BLUE BIRD: visual limitado, lógica/input mantidos em 60 FPS.
...
const VISUAL_FPS_NORMAL: float = 38.0
const VISUAL_FPS_REDUZIDO: float = 23.0
const VISUAL_LIMIAR_REDUZIR_SEG: float = 1.0 / 46.0
const VISUAL_LIMIAR_RECUPERAR_SEG: float = 1.0 / 55.0
const VISUAL_TEMPO_REDUZIR_SEG: float = 0.35
const VISUAL_TEMPO_RECUPERAR_SEG: float = 2.80
const MAX_EVENTOS_ATIVOS: int = 3
const MAX_COMANDOS_LED_POR_TICK: int = 4
const MAX_SISTEMAS_PARTICULAS_ATIVOS: int = 7
...
const VIDEO_FUNDO_ALPHA: float = 0.48

# Proporção real do vídeo (largura / altura) -- AJUSTE pra bater com o naruto.ogv
...
const CAMINHO_IMAGEM_MODAL: String = "res://images/naruto.jpeg"
...
const CORES_ORBE_POR_PONTO: Array = [
	Color(1.0, 0.43, 0.035, 1.0),  # A laranja Naruto
	Color(0.12, 0.64, 1.0, 1.0),   # B azul do céu
	Color(0.18, 0.92, 1.0, 1.0),   # C ciano
	Color(0.08, 0.34, 0.86, 1.0),  # D azul profundo
	Color(1.0, 0.82, 0.18, 1.0),   # E dourado
	Color(0.96, 0.98, 1.0, 1.0),   # F branco azulado
	Color(0.38, 0.72, 1.0, 1.0),   # G azul claro
	Color(1.0, 0.24, 0.10, 1.0),   # H laranja avermelhado
]

const COR_TOQUE_ORBE: Color = Color(1.0, 0.43, 0.035, 1.0)
const COR_SLIDE_AMARELO: Color = Color(1.0, 0.78, 0.16, 1.0)
const COR_SLIDE_PRETO: Color = Color(0.004, 0.018, 0.055, 1.0)
...
var _gradientes_particula_cache: Dictionary = {}
var _material_particula_aditivo_cache: CanvasItemMaterial
var _sistemas_particulas_ativos: Array = []
...
func _ready() -> void:
	...
	_beatmap = BEATMAP_BLUE_BIRD.duplicate(true)
	if not _beatmap.is_empty():
		push_warning(
			"Usando o BEATMAP_BLUE_BIRD escrito à mão -- a detecção de batida em tempo real fica desligada nesta partida."
		)
		...
		push_warning(
			"BEATMAP_BLUE_BIRD vazio -- usando detecção de batida em tempo real (grave/bumbo) da música Blue Bird."
		)
	else:
...
func _desenhar_area_dedicada_central() -> void:
	...
	draw_circle(
		...
		Color(COR_BLUE_BIRD_LARANJA.r, COR_BLUE_BIRD_LARANJA.g, COR_BLUE_BIRD_LARANJA.b, 0.018 + pulso * 0.012)
	)
	...
	draw_arc(
		...
		giro + PI * 1.38,
		72,
		Color(COR_BLUE_BIRD_LARANJA.r, COR_BLUE_BIRD_LARANJA.g, COR_BLUE_BIRD_LARANJA.b, 0.105 + pulso * 0.035),
		2.7,
		true
	)
	draw_arc(
		...
		-giro * 0.82 + PI * 1.72,
		72,
		Color(COR_BLUE_BIRD_AZUL_CEU.r, COR_BLUE_BIRD_AZUL_CEU.g, COR_BLUE_BIRD_AZUL_CEU.b, 0.092 + pulso * 0.032),
		2.8,
		true
	)
	draw_arc(
	...
	for i in range(16 if not _visual_reduzido else 10):
		...
		var cor: Color = COR_BLUE_BIRD_LARANJA.lerp(
			COR_BLUE_BIRD_AZUL_CEU, 0.5 + 0.5 * sin(float(i) * 0.9 + tempo * 2.0)
		)
...
func _desenhar_nucleo_central() -> void:
	...
	draw_circle(
		_centro_tela,
		raio_nucleo * 2.15,
		Color(COR_BLUE_BIRD_AZUL_CEU.r, COR_BLUE_BIRD_AZUL_CEU.g, COR_BLUE_BIRD_AZUL_CEU.b, 0.050 + pulso * 0.050)
	)
	draw_circle(
		_centro_tela,
		raio_nucleo * 1.35,
		Color(COR_BLUE_BIRD_LARANJA.r, COR_BLUE_BIRD_LARANJA.g, COR_BLUE_BIRD_LARANJA.b, 0.035 + pulso * 0.035)
	)
	draw_arc(
		_centro_tela,
		raio_nucleo * 1.82,
	...
	draw_arc(
		...
		-giro + PI * 1.35,
		48,
		Color(COR_BLUE_BIRD_AZUL_CEU.r, COR_BLUE_BIRD_AZUL_CEU.g, COR_BLUE_BIRD_AZUL_CEU.b, 0.68 + pulso * 0.22),
		3.1,
		true
	)
	draw_arc(
		...
		giro * 1.45 + PI * 1.65,
		48,
		Color(COR_BLUE_BIRD_LARANJA.r, COR_BLUE_BIRD_LARANJA.g, COR_BLUE_BIRD_LARANJA.b, 0.70 + pulso * 0.20),
		2.7,
		true
	)
...
func _desenhar_anel_volume() -> void:
	...
	if proporcao > 0.001:
		var cor_nivel: Color = COR_BLUE_BIRD_LARANJA.lerp(COR_ERRO, 1.0 - proporcao)
		draw_arc(
			_centro_tela,
			raio_anel,
			inicio_ang,
...
func _registrar_ponto_rastro_arraste(pos: Vector2) -> void:
	...
	var cor: Color = COR_BLUE_BIRD_LARANJA
...
func _desenhar_rastro_arraste() -> void:
	...
	if _arraste_gesto_ativo:
		var cor_cursor: Color = COR_BLUE_BIRD_LARANJA
		if _arraste_evento_atual != null:
...
func _criar_video_generico(caminho: String, loop: bool) -> void:
	...
	# O áudio oficial do jogo vem do AudioStreamPlayer usando naruto.mp3.
...
# ---------------------------------------------------------------
# 1) APRESENTAÇÃO -- naruto.jpeg + título Blue Bird adaptado ao círculo
# ---------------------------------------------------------------
func _iniciar_apresentacao_fase() -> void:
	...
	# Tamanho calculado para a maior linha ("NARUTO SHIPPUDEN") caber inteira.
	config_nome.font_size = int(_raio_circulo_px * 0.118)
	config_nome.font_color = Color.WHITE
	config_nome.outline_size = max(4, int(_raio_circulo_px * 0.010))
	config_nome.outline_color = Color(0.008, 0.018, 0.060, 0.98)
	config_nome.shadow_size = max(12, int(_raio_circulo_px * 0.030))
	config_nome.shadow_color = Color(
		COR_BLUE_BIRD_AZUL_CEU.r,
		COR_BLUE_BIRD_AZUL_CEU.g,
		COR_BLUE_BIRD_AZUL_CEU.b,
		0.68
	)

	_label_nome_fase = Label.new()
	_label_nome_fase.label_settings = config_nome
	_label_nome_fase.text = TITULO_APRESENTACAO
	...
	_label_nome_fase.clip_text = false
	_label_nome_fase.add_theme_constant_override("line_spacing", -3)

	# Os quatro cantos desta caixa permanecem dentro de aproximadamente 83% do raio.
	# Assim, texto, contorno e sombra nunca escapam da mesa circular.
	_label_nome_fase.size = Vector2(
		_raio_circulo_px * 1.58,
		_raio_circulo_px * 0.48
	)
...
func _iniciar_contagem() -> void:
	...
	config_contagem.shadow_color = Color(COR_BLUE_BIRD_LARANJA.r, COR_BLUE_BIRD_LARANJA.g, COR_BLUE_BIRD_LARANJA.b, 0.7)
...
func _atualizar_leds_guia(tempo_musica: float) -> void:
	# A fita física é EXCLUSIVA dos alvos de clique input_a..input_h.
	# Eventos de arraste continuam visuais na tela, mas nunca acendem D2..D9.
	if Time.get_ticks_msec() < _led_feedback_bloqueio_ate_msec:
		return

	var melhor_evento: Dictionary = {}
	var melhor_distancia: float = 99999.0
	...
	for evento in _eventos_ativos:
		...
		var tempo_alvo: float = float(evento["tempo"])
		if tempo_musica > tempo_alvo + _janela_acerto_evento(evento):
			continue

		var distancia: float = abs(tempo_alvo - tempo_musica)
		if distancia < melhor_distancia:
			melhor_distancia = distancia
	...
	var ponto: int = int(melhor_evento["ponto"])
	var cor: Color = _cor_orbe_por_ponto(ponto)
	_arduino_enviar_unico(
		(
			"LED %d %d %d %d"
			% [
				ponto,
...
void fragment() {
	...
	vec3 azul = vec3(0.16, 0.62, 1.0);
	vec3 rosa = vec3(1.0, 0.24, 0.72);

	// Duas bordas Blue Bird: azul mágico externo e rosa luminoso interno.
	...
	vec3 cor_externa = mix(azul * 0.78, azul, pulso);
	vec3 cor_interna = mix(rosa, azul * 0.92, pulso * 0.18);
	vec3 cor_final = (
		cor_externa * anel_externo * 1.12
		+ cor_interna * anel_interno * 0.98
		+ mix(azul, rosa, 0.35) * halo * 0.14
	);
...
func _criar_hud() -> void:
	...
	estilo.bg_color = Color(0.004, 0.018, 0.055, 0.74)
	estilo.set_border_width_all(2)
	estilo.border_color = Color(COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.86)
	estilo.set_corner_radius_all(int(altura_painel * 0.5))
	estilo.shadow_color = Color(COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.42)
	...
	config_pontuacao.outline_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.86
	)
	...
	estilo_tempo.bg_color = Color(0.004, 0.018, 0.055, 0.77)
	estilo_tempo.set_border_width_all(2)
	estilo_tempo.border_color = Color(COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.94)
	estilo_tempo.set_corner_radius_all(int(altura_tempo * 0.5))
	estilo_tempo.shadow_color = Color(COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.24)
	...
	config_tempo.outline_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.88
	)
	...
	config_combo.font_color = COR_BLUE_BIRD_AZUL_CEU
...
func _mostrar_modal_continuar() -> void:
	...
	estilo.bg_color = Color(0.004, 0.016, 0.045, 0.62)
	estilo.set_border_width_all(4)
	estilo.border_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.98
	)
	estilo.set_corner_radius_all(int(_raio_circulo_px * 0.050))
	estilo.shadow_color = Color(
		COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.24
	)
	estilo.shadow_size = 26
	_modal_painel.add_theme_stylebox_override("panel", estilo)
	_modal_layer.add_child(_modal_painel)

	# Fundo temático do resultado. Usa exatamente naruto.jpeg e mantém a
	...
	estilo_moldura.border_color = Color(
		COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.74
	)
	...
	# Cabeçalho Blue Bird em vidro azul/rosa.
	...
	estilo_faixa.bg_color = Color(
		COR_BLUE_BIRD_AZUL.r * 0.24,
		COR_BLUE_BIRD_AZUL.g * 0.24,
		COR_BLUE_BIRD_AZUL.b * 0.24,
		0.82
	)
	estilo_faixa.set_border_width_all(2)
	estilo_faixa.border_color = Color(
		COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.82
	)
	estilo_faixa.set_corner_radius_all(int(altura * 0.055))
	estilo_faixa.shadow_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.42
	)
	...
	cfg_titulo.font_color = COR_BLUE_BIRD_AZUL_CLARO
	cfg_titulo.outline_size = 5
	cfg_titulo.outline_color = Color(0.006, 0.030, 0.090, 0.96)
	cfg_titulo.shadow_size = 15
	cfg_titulo.shadow_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.64
	)

	_modal_label_titulo = Label.new()
	_modal_label_titulo.label_settings = cfg_titulo
	_modal_label_titulo.text = NOME_FASE
	...
	cfg_subtitulo.font_color = Color(0.95, 0.90, 1.0, 1.0)
	...
	cfg_score.font_color = COR_BLUE_BIRD_AZUL_CLARO
	cfg_score.outline_size = 6
	cfg_score.outline_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.96
	)
	cfg_score.shadow_size = 18
	cfg_score.shadow_color = Color(
		COR_BLUE_BIRD_AZUL.r, COR_BLUE_BIRD_AZUL.g, COR_BLUE_BIRD_AZUL.b, 0.52
	)
	...
	barra_cheia.shadow_color = Color(
		COR_BLUE_BIRD_AZUL_CLARO.r, COR_BLUE_BIRD_AZUL_CLARO.g, COR_BLUE_BIRD_AZUL_CLARO.b, 0.34
	)
	...
	cfg_acao.font_color = COR_BLUE_BIRD_AZUL_CLARO
...
func _criar_evento_musical_dinamico(dados: Dictionary) -> void:
	if _fila_eventos_musicais.size() >= ANALISE_FILA_MAX:
		return
...
func _despachar_fila_eventos_musicais(tempo_musica: float) -> void:
	...
	while not _fila_eventos_musicais.is_empty() and _eventos_ativos.size() < MAX_EVENTOS_ATIVOS:
		var evento: Dictionary = _fila_eventos_musicais.pop_front()
		if evento.get("tipo", "") == "arraste" and _existe_arraste_ativo():
			# Mantém o evento na frente para não criar dois tubos simultâneos.
			_fila_eventos_musicais.push_front(evento)
			break
...
func _ativar_eventos_pendentes(tempo_musica: float) -> void:
	while _indice_beatmap < _beatmap.size() and _eventos_ativos.size() < MAX_EVENTOS_ATIVOS:
		var proximo: Dictionary = _beatmap[_indice_beatmap]
		if (float(proximo["tempo"]) - TEMPO_APROXIMACAO_SEG) > tempo_musica:
			break

		if proximo.get("tipo", "") == "arraste" and _existe_arraste_ativo():
			var expirou: bool = (
				proximo.has("tempo_fim")
				and tempo_musica > float(proximo["tempo_fim"]) + ARRASTE_TOLERANCIA_FINAL_SEG
			)
			if expirou:
				_indice_beatmap += 1
				continue
...
func _atualizar_eventos_ativos(tempo_musica: float) -> void:
	for i in range(_eventos_ativos.size() - 1, -1, -1):
		...
		if evento["tipo"] == "toque":
			...
			var venceu_janela: bool = tempo_musica > evento["tempo"] + _janela_acerto_evento(evento)
...
func _processar_inputs_toque(tempo_musica: float) -> void:
	for i in range(NUM_PONTOS):
		...
		if Input.is_action_just_pressed(acao):
			# Confirma visualmente e no LED exatamente o input físico que foi pressionado.
			...
			_arduino_enviar_forcado("PULSE " + str(i))
...
func _tentar_resolver_toque(indice_ponto: int, tempo_musica: float) -> void:
	for evento in _eventos_ativos:
		if (
			...
			if abs(tempo_musica - evento["tempo"]) <= _janela_acerto_evento(evento):
				evento["resolvido"] = true
				_registrar_acerto(PONTOS_POR_ACERTO_TOQUE)
				var ponto: Node2D = _pontos[indice_ponto]
				_ponto_set_estado(indice_ponto, "hig", 0.16, COR_ACERTO)
				...
				_led_feedback_bloqueio_ate_msec = Time.get_ticks_msec() + 260
				_arduino_enviar_forcado("OK " + str(indice_ponto))
				return
...
func _registrar_erro(_evento: Dictionary) -> void:
	...
	if _evento.get("tipo", "") == "toque":
		...
		if indice_led >= 0:
			...
			_arduino_enviar_forcado("ERR " + str(indice_led))
...
func _gerar_gradiente_particula(cor_base: Color) -> Gradient:
	# O gradiente é reutilizado por cor para evitar alocações em cada acerto.
	var chave: String = cor_base.to_html(true)
	if _gradientes_particula_cache.has(chave):
		return _gradientes_particula_cache[chave]
	...
	_gradientes_particula_cache[chave] = grad
	return grad


func _obter_material_particula_aditivo() -> CanvasItemMaterial:
	if _material_particula_aditivo_cache != null:
		return _material_particula_aditivo_cache

	_material_particula_aditivo_cache = CanvasItemMaterial.new()
	_material_particula_aditivo_cache.blend_mode = CanvasItemMaterial.BLEND_MODE_ADD
	return _material_particula_aditivo_cache
...
func _explosao_particulas(pos: Vector2, cor: Color) -> void:
	...
	_criar_particulas_neon(
		pos, cor, 10 if not _visual_reduzido else 6, 0.36, 34.0, 92.0, 0.18, 0.34, 24
	)

	if not _visual_reduzido:
		var cor_extra: Color = cor.lerp(Color(1.0, 0.24, 0.72, 1.0), 0.38)
		_criar_particulas_neon(pos, cor_extra, 4, 0.25, 22.0, 62.0, 0.09, 0.20, 25)
...
func _criar_particulas_neon(
	...
	# Remove referências encerradas e limita sistemas simultâneos.
	for i in range(_sistemas_particulas_ativos.size() - 1, -1, -1):
		if not is_instance_valid(_sistemas_particulas_ativos[i]):
			_sistemas_particulas_ativos.remove_at(i)

	while _sistemas_particulas_ativos.size() >= MAX_SISTEMAS_PARTICULAS_ATIVOS:
		var antigo: Node = _sistemas_particulas_ativos.pop_front()
	...
	particulas.material = _obter_material_particula_aditivo()

	add_child(particulas)
	_sistemas_particulas_ativos.append(particulas)
	particulas.finished.connect(
		func() -> void:
			_sistemas_particulas_ativos.erase(particulas)
...
# ---------------------------------------------------------------
# BEATMAP DE TESTE (fallback; o motor dinâmico usa BEATMAP_BLUE_BIRD vazio)
# ---------------------------------------------------------------


func _gerar_beatmap_blue_bird_por_fases(duracao_seg: float) -> Array:
	var eventos: Array = []
	...
	#
	# A música real desta fase é naruto.mp3.

extends Node2D
## ============================================================
## PLAY — HIT MUSIC — DRAGON BALL (mesa redonda / tela circular)
##
...
##
## 1. APRESENTACAO -- somente o nome da fase, sem imagem de capa.
## 2. CONTAGEM -- vídeo dragon-ball.ogv com contagem regressiva por cima.
## 3. JOGANDO -- vídeo dragon-ball.ogv centralizado no círculo, com
##    os 8 pontos, HUD, música dragon-ball e beatmap musical aleatório/adaptativo.
##
## Dentro de JOGANDO:
...
## - CAMINHO_MUSICA usa "res://songs/dragon-ball.mp3".
##   (ajuste a extensão/nome se o arquivo de áudio real tiver outro nome).
## - CAMINHO_VIDEO_CONTAGEM usa "res://medias/dragon-ball.ogv".
## - CAMINHO_VIDEO_FUNDO usa "res://medias/dragon-ball.ogv".
...
## - NOME_FASE está como "DRAGON BALL".
## - BEATMAP_DRAGON_BALL pode ficar vazio: o motor multibanda escuta a música
##   em tempo real e cria toques/arrastes conforme pulsação, energia e timbre.
## - Esta cena usa a mecânica completa da fase Soul Eater como base,
##   mas com identidade visual e comportamento rítmico próprios de Dragon Ball:
##   saltos largos, sequências em portais e arrastes mais complexos.
## - Use dragon-ball.ogv SEM faixa de áudio e dragon-ball.mp3 separado.
##   Assim os efeitos completos permanecem ativos sem o peso duplicado do áudio do vídeo.
## ============================================================
...
# --- Apresentação da fase: apenas nome, sem imagem de capa ---
const NOME_FASE: String = "DRAGON BALL"
const TITULO_APRESENTACAO: String = "DRAGON BALL"
const DURACAO_APRESENTACAO_SEG: float = 2.5

# --- Contagem regressiva de início ---
const CAMINHO_VIDEO_CONTAGEM: String = "res://medias/dragon-ball.ogv"
...
const CAMINHO_VIDEO_FUNDO: String = "res://medias/dragon-ball.ogv"

# Fallback automático: se você escreveu a pasta como medias em vez de mmedias,
# o script também tenta encontrar o vídeo em res://medias/dragon-ball.ogv.
const CAMINHO_VIDEO_FALLBACK_1: String = "res://medias/dragon-ball.ogv"

# --- Música ---
const CAMINHO_MUSICA: String = "res://songs/dragon-ball.mp3"
...
const JANELA_PERFEITO_SEG: float = 0.075
...
const COR_LED_PERFEITO: Color = Color(0.32, 1.0, 0.58)
const COR_LED_BOM: Color = Color(1.0, 0.82, 0.06)
...
const PERFIL_JOGABILIDADE: Dictionary = {
	# Perfil energético: bateria e ataques fortes geram toques, enquanto mudanças
	# melódicas e compassos marcantes abrem sequências Saiyajin em zigue-zague.
	"nome": "DRAGON_BALL_SAIYAJIN_ENERGETICO",
	"sensibilidade": 1.14,
	"densidade": 0.91,
	"preferencia_arraste": 0.24,
	"complexidade_arraste": 0.68,
	"distancia_saltos": 0.90,
	"janela_toque_extra": 0.018,
	"compasso_arraste": 4,
	"chance_toque_duplo": 0.26,
	"intervalo_toque_duplo": 0.14,
	"duracao_arraste_mul": 1.10,
	"estilo_saiyajin": true,
}

# --- Cores neon Dragon Ball: laranja, azul de energia e amarelo Super Saiyajin. ---
const COR_NEON_VERDE: Color = Color(1.0, 0.32, 0.035)
const COR_NEON_CIANO: Color = Color(0.08, 0.62, 1.0)
const COR_ACERTO: Color = Color(1.0, 0.88, 0.12)
const COR_ERRO: Color = Color(1.0, 0.12, 0.18)
...
## ------------------------------------------------------------
const BEATMAP_DRAGON_BALL: Array = [
...
## Se BEATMAP_DRAGON_BALL estiver vazio, o jogo não usa mais fases fixas por
...
# Ajuste vertical herdado da tela Soul, aplicado à fase Dragon Ball.
...
# Identidade visual Dragon Ball: laranja do uniforme, azul de Ki e dourado Saiyajin.
const COR_DRAGON_LARANJA: Color = Color(1.0, 0.30, 0.025, 1.0)
const COR_DRAGON_CLARO: Color = Color(1.0, 0.97, 0.86, 1.0)
const COR_DRAGON_AZUL: Color = Color(0.06, 0.58, 1.0, 1.0)
const COR_DRAGON_DOURADO: Color = Color(1.0, 0.86, 0.08, 1.0)
...
# A lógica/input continua em 60 FPS. O redesenho dos efeitos Node2D é desacoplado
# apenas como proteção leve. Nenhum efeito é eliminado: a mecânica visual completa
# da Soul permanece ativa porque o OGV deve ser usado sem áudio interno.
...
# Proporção real do vídeo (largura / altura) -- AJUSTE pra bater com o dragon-ball.ogv
...
const CAMINHO_IMAGEM_MODAL: String = "res://images/dragon-ball.jpeg"
...
# Dragon Ball usa exclusivamente amarelo/dourado e laranja nos orbes e LEDs.
const CORES_ORBE_POR_PONTO: Array = [
	Color(1.0, 0.88, 0.055, 1.0), # A amarelo
	Color(1.0, 0.30, 0.025, 1.0), # B laranja
	Color(1.0, 0.88, 0.055, 1.0), # C amarelo
	Color(1.0, 0.30, 0.025, 1.0), # D laranja
	Color(1.0, 0.88, 0.055, 1.0), # E amarelo
	Color(1.0, 0.30, 0.025, 1.0), # F laranja
	Color(1.0, 0.88, 0.055, 1.0), # G amarelo
	Color(1.0, 0.30, 0.025, 1.0), # H laranja
]

const COR_TOQUE_ORBE: Color = Color(1.0, 0.30, 0.025, 1.0)
const COR_SLIDE_AMARELO: Color = Color(1.0, 0.88, 0.055, 1.0)
const COR_SLIDE_PRETO: Color = Color(0.055, 0.012, 0.0, 1.0)
const COR_SLIDE_COMPLETO: Color = Color(1.0, 0.68, 0.035, 1.0)
...
func _ready() -> void:
	...
	_beatmap = BEATMAP_DRAGON_BALL.duplicate(true)
	if not _beatmap.is_empty():
		push_warning(
			"Usando o BEATMAP_DRAGON_BALL escrito à mão -- a detecção de batida em tempo real fica desligada nesta partida."
		)
		...
		push_warning(
			"BEATMAP_DRAGON_BALL vazio -- usando detecção de batida em tempo real (grave/bumbo) da música dragon-ball."
		)
	else:
...
func _atualizar_governador_visual(delta: float) -> void:
	# Proteção leve: reduz somente a frequência de redesenho durante uma queda real.
	# Não existe mais modo crítico e nenhum efeito da jogabilidade é removido.
...
func _criar_video_generico(caminho: String, loop: bool) -> void:
	...
	# O áudio oficial do jogo vem do AudioStreamPlayer usando dragon-ball.mp3 separado.
...
func _iniciar_apresentacao_fase() -> void:
	...
	_label_nome_fase.size = Vector2(_raio_circulo_px * 1.65, _raio_circulo_px * 0.52)
...
func _iniciar_contagem() -> void:
	...
	_criar_video_generico(CAMINHO_VIDEO_CONTAGEM, true)
...
func _iniciar_jogo_real() -> void:
	...
	elif not _video_fundo.is_playing():
		# Se o arquivo terminou durante a contagem, retoma imediatamente sem recriar o nó.
		_video_fundo.play()
	if _musica_player == null or not is_instance_valid(_musica_player):
		_tocar_musica()
...
func _led_enviar_cor_independente(ponto: int, cor: Color) -> void:
	if ponto < 0 or ponto >= NUM_PONTOS:
		return
	var comando: String = "LED %d %d %d %d" % [ponto, int(cor.r * 255.0), int(cor.g * 255.0), int(cor.b * 255.0)]
...
func _led_evento_adquirir(evento: Dictionary) -> void:
	...
	evento["led_fim_tempo_musica"] = float(evento.get("tempo", 0.0)) + _janela_acerto_evento(evento)
...
func _led_sincronizar_com_discos_vivos(tempo_musica: float) -> void:
	...
	for evento in _eventos_ativos:
		...
		if ponto < 0 or ponto >= NUM_PONTOS:
			continue
		var alvo: float = float(evento.get("tempo", 0.0))
		var inicio: float = float(evento.get("tempo_visual_inicio", alvo - TEMPO_ORBE_SEG))
		var fim: float = float(evento.get("led_fim_tempo_musica", alvo + _janela_acerto_evento(evento)))
	...
	for ponto in range(NUM_PONTOS):
		...
		_led_enviar_cor_independente(ponto, _cor_led_guia(ponto) if vivos_por_ponto[ponto] > 0 else Color.BLACK)
	...
	if ponte_pronta:
		if not _serial_pronta_anunciada:
			...
			for i in range(_led_assinaturas_por_ponto.size()):
				...
				_led_enviar_cor_independente(i, _cor_led_guia(i) if _led_donos_por_ponto[i] > 0 else Color.BLACK)
...
void fragment() {
	...
	vec3 laranja_dragon = vec3(1.0, 0.30, 0.025);
	vec3 azul_ki = vec3(0.06, 0.58, 1.0);
	vec3 dourado = vec3(1.0, 0.86, 0.08);

	// Duas bordas Dragon Ball: laranja externo e azul de Ki interno.
	...
	vec3 cor_externa = mix(laranja_dragon * 0.72, laranja_dragon, pulso);
	vec3 cor_interna = mix(azul_ki, vec3(1.0, 0.97, 0.86), pulso * 0.24);
	vec3 cor_final = (
		cor_externa * anel_externo * 1.14
...
func _criar_hud() -> void:
	...
	estilo.bg_color = Color(0.025, 0.0, 0.055, 0.74)
	estilo.set_border_width_all(2)
	estilo.border_color = Color(COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.86)
	estilo.set_corner_radius_all(int(altura_painel * 0.5))
	estilo.shadow_color = Color(COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.42)
	...
	config_pontuacao.outline_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.86
	)
	...
	estilo_tempo.bg_color = Color(0.025, 0.0, 0.055, 0.77)
	estilo_tempo.set_border_width_all(2)
	estilo_tempo.border_color = Color(COR_DRAGON_AZUL.r, COR_DRAGON_AZUL.g, COR_DRAGON_AZUL.b, 0.94)
	estilo_tempo.set_corner_radius_all(int(altura_tempo * 0.5))
	estilo_tempo.shadow_color = Color(COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.24)
	...
	config_tempo.outline_color = Color(
		COR_DRAGON_AZUL.r, COR_DRAGON_AZUL.g, COR_DRAGON_AZUL.b, 0.88
	)
...
func _mostrar_modal_continuar() -> void:
	...
	estilo.bg_color = Color(0.018, 0.002, 0.040, 0.62)
	estilo.set_border_width_all(4)
	estilo.border_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.98
	)
	estilo.set_corner_radius_all(int(_raio_circulo_px * 0.050))
	estilo.shadow_color = Color(
		COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.24
	)
	estilo.shadow_size = 26
	_modal_painel.add_theme_stylebox_override("panel", estilo)
	_modal_layer.add_child(_modal_painel)

	# Fundo temático do resultado. Usa a arte Dragon Ball e mantém a
	...
	estilo_moldura.border_color = Color(
		COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.74
	)
	...
	# Cabeçalho Dragon Ball em vidro de saiyajin verde/ciano.
	...
	estilo_faixa.bg_color = Color(
		COR_DRAGON_LARANJA.r * 0.24,
		COR_DRAGON_LARANJA.g * 0.24,
		COR_DRAGON_LARANJA.b * 0.24,
		0.82
	)
	estilo_faixa.set_border_width_all(2)
	estilo_faixa.border_color = Color(
		COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.82
	)
	estilo_faixa.set_corner_radius_all(int(altura * 0.055))
	estilo_faixa.shadow_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.42
	)
	...
	cfg_titulo.font_color = COR_DRAGON_CLARO
	cfg_titulo.outline_size = 5
	cfg_titulo.outline_color = Color(0.07, 0.0, 0.12, 0.96)
	cfg_titulo.shadow_size = 15
	cfg_titulo.shadow_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.64
	)

	_modal_label_titulo = Label.new()
	_modal_label_titulo.label_settings = cfg_titulo
	_modal_label_titulo.text = "DRAGON BALL"
	...
	label_subtitulo.text = "DRAGON BALL • RESULTADO FINAL"
	...
	cfg_score.font_color = COR_DRAGON_DOURADO
	cfg_score.outline_size = 6
	cfg_score.outline_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.96
	)
	cfg_score.shadow_size = 18
	cfg_score.shadow_color = Color(
		COR_DRAGON_LARANJA.r, COR_DRAGON_LARANJA.g, COR_DRAGON_LARANJA.b, 0.52
	)
	...
	barra_cheia.shadow_color = Color(
		COR_DRAGON_CLARO.r, COR_DRAGON_CLARO.g, COR_DRAGON_CLARO.b, 0.34
	)
	...
	cfg_acao.font_color = COR_DRAGON_CLARO
	...
	_criar_evento_musical_dinamico(
		{
			...
			"pico_transiente": pico_transiente,
...
func _criar_evento_musical_dinamico(dados: Dictionary) -> void:
	...
	var compasso_arraste: int = max(int(PERFIL_JOGABILIDADE.get("compasso_arraste", 4)), 1)
	var compasso_forte: bool = _contador_batidas % compasso_arraste == 0
	...
	if usar_arraste:
		...
		var duracao_mul: float = clamp(
			float(PERFIL_JOGABILIDADE.get("duracao_arraste_mul", 1.0)), 0.75, 1.35
		)
		var duracao: float = clamp(
			_intervalo_beat_estimado * (2.15 + float(tamanho - 2) * 0.55) * duracao_mul,
			0.92,
			2.55
		)
		(
		...
		# Identidade Dragon Ball: ataques secos e compassos fortes podem abrir
		# dois portais em sequência curta, sempre em pontos distantes e jogáveis.
		var chance_duplo: float = clamp(
			float(PERFIL_JOGABILIDADE.get("chance_toque_duplo", 0.0)), 0.0, 0.65
		)
		var pico_transiente: bool = bool(dados.get("pico_transiente", false))
		var ataque_saiyajin: bool = (
			(pico_transiente and intensidade > 1.32)
			or (compasso_forte and intensidade > 1.55 and corpo_melodico < 1.42)
		)
		if (
			bool(PERFIL_JOGABILIDADE.get("estilo_saiyajin", false))
			and ataque_saiyajin
			and _rng.randf() < chance_duplo
		):
			var intervalo_duplo: float = clamp(
				float(PERFIL_JOGABILIDADE.get("intervalo_toque_duplo", 0.14)), 0.10, 0.22
			)
			var salto_duplo: int = 4 if intensidade > 1.85 else 3
			var ponto_duplo: int = posmod(
				ponto + _direcao_musical * salto_duplo, NUM_PONTOS
			)
			if ponto_duplo == ponto:
				ponto_duplo = posmod(ponto + 4, NUM_PONTOS)
			_fila_eventos_musicais.append(
				{
					"tipo": "toque",
					"tempo": tempo_alvo + intervalo_duplo,
					"tempo_visual_inicio": tempo_detectado,
					"ponto": ponto_duplo,
					"resolvido": false,
					"via_beat": true,
			...
			_ultimo_ponto_beat = ponto_duplo
		else:
...
func _montar_caminho_arraste_dinamico(
	...
	var estilo_saiyajin: bool = bool(PERFIL_JOGABILIDADE.get("estilo_saiyajin", false))
	var distancia_perfil: float = clamp(
		float(PERFIL_JOGABILIDADE.get("distancia_saltos", 0.72)), 0.0, 1.0
	)
	for passo in range(1, tamanho):
		if passo % 2 == 0 or corpo_melodico > 1.45 or (estilo_saiyajin and passo > 1):
			direcao *= -1
		var salto_base: float = (
			1.65
			+ distancia_perfil * 1.15
			+ (intensidade - 1.0) * 0.55
			+ float(passo % 2) * 0.55
		)
		var salto: int = clamp(int(round(salto_base)), 2, 4)
...
func _ativar_eventos_pendentes(tempo_musica: float) -> void:
	while _indice_beatmap < _beatmap.size():
		...
		evento["tempo_visual_inicio"] = float(evento.get("tempo_visual_inicio", float(evento["tempo"]) - TEMPO_ORBE_SEG))
		_eventos_ativos.append(evento)
		_led_evento_adquirir(evento)
		_indice_beatmap += 1
...
func _atualizar_eventos_ativos(tempo_musica: float) -> void:
	for i in range(_eventos_ativos.size() - 1, -1, -1):
		...
		if evento["tipo"] == "toque":
			...
			var fim_individual: float = float(evento.get("led_fim_tempo_musica", float(evento["tempo"]) + _janela_acerto_evento(evento)))
			if tempo_musica > fim_individual:
...
func _processar_inputs_toque(tempo_musica: float) -> void:
	for i in range(NUM_PONTOS):
		...
		if Input.is_action_just_pressed(acao):
			# O julgamento controla o resultado; não há comando serial que interrompa outros LEDs.
...
func _tentar_resolver_toque(indice_ponto: int, tempo_musica: float) -> void:
	...
	_explosao_particulas(ponto.position, cor_resultado)
	_efeito_texto_flutuante(texto_resultado, ponto.position, cor_resultado)
...
func _registrar_erro(_evento: Dictionary) -> void:
	...
	# O erro é visual. O estado físico continua derivado exclusivamente dos discos vivos.
	if _evento.get("tipo", "") == "toque":
		var indice_led: int = int(_evento.get("ponto", -1))
		if indice_led >= 0 and indice_led < _pontos.size():
...
# ---------------------------------------------------------------
# BEATMAP DE TESTE (fallback; o motor dinâmico usa BEATMAP_DRAGON_BALL vazio)
# ---------------------------------------------------------------


func _gerar_beatmap_dragon_ball_por_fases(duracao_seg: float) -> Array:
	var eventos: Array = []
	...
	#
	# A música real desta fase é dragon-ball.mp3.

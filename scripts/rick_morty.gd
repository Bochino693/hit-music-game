extends Node2D
## ============================================================
## PLAY — HIT MUSIC — LOVE ENTROPY / RICK AND MORTY (mesa redonda / tela circular)
##
...
## 2. CONTAGEM -- vídeo rick_morty.ogv com contagem regressiva por cima.
## 3. JOGANDO -- vídeo rick_morty.ogv centralizado no círculo, com
##    os 8 pontos, HUD, música rick_morty e beatmap musical aleatório/adaptativo.
##
## Dentro de JOGANDO:
...
## - CAMINHO_MUSICA usa "res://songs/rick_morty.mp3".
##   (ajuste a extensão/nome se o arquivo de áudio real tiver outro nome).
## - CAMINHO_IMAGEM_FASE usa "res://images/rick_morty.png".
## - CAMINHO_VIDEO_CONTAGEM usa "res://medias/rick_morty.ogv".
## - CAMINHO_VIDEO_FUNDO usa "res://medias/rick_morty.ogv".
...
## - NOME_FASE está como "LOVE ENTROPY - RICK AND MORTY ANIME".
## - BEATMAP_RICK_MORTY pode ficar vazio: o motor multibanda escuta a música
...
##   mas com identidade visual e comportamento rítmico próprios de Rick and Morty:
##   saltos largos, sequências em portais e arrastes mais complexos.
## - Use rick_morty.ogv SEM faixa de áudio e rick_morty.mp3 separado.
...
const CAMINHO_IMAGEM_FASE: String = "res://images/rick_morty.png"
const NOME_FASE: String = "LOVE ENTROPY - RICK AND MORTY ANIME"
const TITULO_APRESENTACAO: String = "LOVE ENTROPY\nRICK AND MORTY ANIME"
const DURACAO_APRESENTACAO_SEG: float = 2.5

# --- Contagem regressiva de início ---
const CAMINHO_VIDEO_CONTAGEM: String = "res://medias/rick_morty.ogv"
...
const CAMINHO_VIDEO_FUNDO: String = "res://medias/rick_morty.ogv"

# Fallback automático: se você escreveu a pasta como medias em vez de mmedias,
# o script também tenta encontrar o vídeo em res://medias/rick_morty.ogv.
const CAMINHO_VIDEO_FALLBACK_1: String = "res://medias/rick_morty.ogv"

# --- Música ---
const CAMINHO_MUSICA: String = "res://songs/rick_morty.mp3"
...
const PERFIL_JOGABILIDADE: Dictionary = {
	# LOVE ENTROPY usa uma leitura eletrônico-cinemática: graves geram toques fortes,
	# ataques secos podem abrir pares rápidos e mudanças melódicas criam “portais”
	# longos em zigue-zague. Assim ela preserva a mecânica da Soul, mas joga diferente.
	"nome": "LOVE_ENTROPY_PORTAL_FLOW",
	"sensibilidade": 1.08,
	"densidade": 0.88,
	"preferencia_arraste": 0.52,
	"complexidade_arraste": 0.86,
	"distancia_saltos": 0.94,
	"janela_toque_extra": 0.018,
	"compasso_arraste": 4,
	"chance_toque_duplo": 0.30,
	"intervalo_toque_duplo": 0.14,
	"duracao_arraste_mul": 1.10,
	"estilo_portal": true,
}

# --- Cores neon (Rick and Morty: verde portal, ciano, ácido e vermelho de falha) ---
const COR_NEON_VERDE: Color = Color(0.14, 1.0, 0.42)
const COR_NEON_CIANO: Color = Color(0.08, 0.88, 1.0)
const COR_ACERTO: Color = Color(0.72, 1.0, 0.18)
...
## ------------------------------------------------------------
const BEATMAP_RICK_MORTY: Array = [
...
## Se BEATMAP_RICK_MORTY estiver vazio, o jogo não usa mais fases fixas por
...
# Ajuste vertical herdado da tela Soul, aplicado à fase Rick and Morty.
...
# Identidade visual Rick and Morty: verde portal, ciano tecnológico e branco ácido.
const COR_RICK_PORTAL: Color = Color(0.14, 1.0, 0.42, 1.0)
const COR_RICK_CLARO: Color = Color(0.92, 1.0, 0.97, 1.0)
const COR_RICK_CIANO: Color = Color(0.08, 0.88, 1.0, 1.0)
const COR_RICK_ACIDO: Color = Color(0.90, 1.0, 0.16, 1.0)
...
# Proporção real do vídeo (largura / altura) -- AJUSTE pra bater com o rick_morty.ogv
...
const CAMINHO_IMAGEM_MODAL: String = "res://images/rick_morty.jpeg"
...
const CORES_ORBE_POR_PONTO: Array = [
	Color(0.14, 1.0, 0.42, 1.0),  # A verde portal
	Color(0.08, 0.88, 1.0, 1.0), # B ciano dimensional
	Color(0.90, 1.0, 0.16, 1.0), # C amarelo ácido
	Color(0.18, 0.48, 1.0, 1.0), # D azul elétrico
	Color(0.68, 0.24, 1.0, 1.0), # E roxo multiverso
	Color(1.0, 0.38, 0.10, 1.0), # F laranja energético
	Color(1.0, 0.18, 0.72, 1.0), # G rosa cósmico
	Color(1.0, 0.10, 0.18, 1.0), # H vermelho de alerta
]

const COR_TOQUE_ORBE: Color = Color(0.14, 1.0, 0.42, 1.0)
const COR_SLIDE_AMARELO: Color = Color(0.90, 1.0, 0.16, 1.0)
const COR_SLIDE_PRETO: Color = Color(0.008, 0.018, 0.014, 1.0)
const COR_SLIDE_COMPLETO: Color = Color(0.08, 0.95, 0.68, 1.0)
...
func _ready() -> void:
	...
	_beatmap = BEATMAP_RICK_MORTY.duplicate(true)
	if not _beatmap.is_empty():
		push_warning(
			"Usando o BEATMAP_RICK_MORTY escrito à mão -- a detecção de batida em tempo real fica desligada nesta partida."
		)
		...
		push_warning(
			"BEATMAP_RICK_MORTY vazio -- usando detecção de batida em tempo real (grave/bumbo) da música rick_morty."
		)
	else:
...
func _criar_video_generico(caminho: String, loop: bool) -> void:
	...
	# O áudio oficial do jogo vem do AudioStreamPlayer usando rick_morty.mp3 separado.
...
# ---------------------------------------------------------------
# 1) APRESENTAÇÃO -- rick_morty.png + nome da fase
# ---------------------------------------------------------------
func _iniciar_apresentacao_fase() -> void:
	...
	# O título mais longo fica em duas linhas e usa uma escala segura para a mesa circular.
	config_nome.font_size = int(_raio_circulo_px * 0.108)
	config_nome.font_color = Color.WHITE
	config_nome.outline_size = max(4, int(_raio_circulo_px * 0.010))
	config_nome.outline_color = Color(0.008, 0.022, 0.016, 0.98)
	...
	# A caixa inteira, incluindo contorno e sombra, permanece dentro da margem segura
	# do círculo. Nenhuma palavra é escondida ou posicionada fora da mesa.
...
void fragment() {
	...
	vec3 verde_portal = vec3(0.14, 1.0, 0.42);
	vec3 ciano = vec3(0.08, 0.88, 1.0);
	vec3 acido = vec3(0.90, 1.0, 0.16);

	// Duas bordas de portal: verde externo e ciano/branco interno.
	...
	vec3 cor_externa = mix(verde_portal * 0.72, verde_portal, pulso);
	vec3 cor_interna = mix(ciano, vec3(0.94, 1.0, 0.98), pulso * 0.24);
	vec3 cor_final = (
...
func _criar_hud() -> void:
	...
	estilo.border_color = Color(COR_RICK_CLARO.r, COR_RICK_CLARO.g, COR_RICK_CLARO.b, 0.86)
	estilo.set_corner_radius_all(int(altura_painel * 0.5))
	estilo.shadow_color = Color(COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.42)
	...
	config_pontuacao.outline_color = Color(
		COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.86
	)
	...
	estilo_tempo.border_color = Color(COR_RICK_CIANO.r, COR_RICK_CIANO.g, COR_RICK_CIANO.b, 0.94)
	estilo_tempo.set_corner_radius_all(int(altura_tempo * 0.5))
	estilo_tempo.shadow_color = Color(COR_RICK_CLARO.r, COR_RICK_CLARO.g, COR_RICK_CLARO.b, 0.24)
	...
	config_tempo.outline_color = Color(
		COR_RICK_CIANO.r, COR_RICK_CIANO.g, COR_RICK_CIANO.b, 0.88
	)
...
func _mostrar_modal_continuar() -> void:
	...
	# Moldura interna clara sobre a borda verde portal.
	...
	estilo_moldura.border_color = Color(
		COR_RICK_CLARO.r, COR_RICK_CLARO.g, COR_RICK_CLARO.b, 0.74
	)
	...
	# Cabeçalho Rick and Morty em vidro de portal verde/ciano.
	...
	estilo_faixa.bg_color = Color(
		COR_RICK_PORTAL.r * 0.24,
		COR_RICK_PORTAL.g * 0.24,
		COR_RICK_PORTAL.b * 0.24,
		0.82
	)
	estilo_faixa.set_border_width_all(2)
	estilo_faixa.border_color = Color(
		COR_RICK_CLARO.r, COR_RICK_CLARO.g, COR_RICK_CLARO.b, 0.82
	)
	estilo_faixa.set_corner_radius_all(int(altura * 0.055))
	estilo_faixa.shadow_color = Color(
		COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.42
	)
	...
	cfg_titulo.font_color = COR_RICK_CLARO
	...
	cfg_titulo.shadow_color = Color(
		COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.64
	)

	_modal_label_titulo = Label.new()
	_modal_label_titulo.label_settings = cfg_titulo
	_modal_label_titulo.text = "RICK AND MORTY"
	...
	label_subtitulo.text = "LOVE ENTROPY • RESULTADO FINAL"
	...
	cfg_score.font_color = COR_RICK_ACIDO
	cfg_score.outline_size = 6
	cfg_score.outline_color = Color(
		COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.96
	)
	cfg_score.shadow_size = 18
	cfg_score.shadow_color = Color(
		COR_RICK_PORTAL.r, COR_RICK_PORTAL.g, COR_RICK_PORTAL.b, 0.52
	)
	...
	barra_cheia.shadow_color = Color(
		COR_RICK_CLARO.r, COR_RICK_CLARO.g, COR_RICK_CLARO.b, 0.34
	)
	...
	cfg_acao.font_color = COR_RICK_CLARO
...
func _criar_evento_musical_dinamico(dados: Dictionary) -> void:
	...
	if usar_arraste:
		...
		# Identidade Rick and Morty: ataques secos e compassos fortes podem abrir
...
func _montar_caminho_arraste_dinamico(
	...
	var estilo_portal: bool = bool(PERFIL_JOGABILIDADE.get("estilo_portal", false))
	var distancia_perfil: float = clamp(
		float(PERFIL_JOGABILIDADE.get("distancia_saltos", 0.72)), 0.0, 1.0
	)
	for passo in range(1, tamanho):
		if passo % 2 == 0 or corpo_melodico > 1.45 or (estilo_portal and passo > 1):
			direcao *= -1
...
# ---------------------------------------------------------------
# BEATMAP DE TESTE (fallback; o motor dinâmico usa BEATMAP_RICK_MORTY vazio)
# ---------------------------------------------------------------


func _gerar_beatmap_rick_morty_por_fases(duracao_seg: float) -> Array:
	var eventos: Array = []
	...
	#
	# A música real desta fase é rick_morty.mp3.

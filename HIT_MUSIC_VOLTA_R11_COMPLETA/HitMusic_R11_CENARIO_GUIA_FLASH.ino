/*
  HIT MUSIC R11 - CENARIO + GUIA + FLASH
  Baseado no R10, corrigindo:
  1) LEDs do jogo perdiam a cena ao iniciar.
  2) O comando LED desativava o fundo tematico.
  3) Guias e flashes nao conviviam com o SCENE.
  4) D2/D3 do menu precisavam continuar firmes.
*/
...
void desenhar_leds() {
  ...
  if (!desenho_sujo && !animando) {
    return;
  }

  if (animando && agora - ultimo_frame_led_ms < INTERVALO_FRAME_LED_MS) {
    return;
  }
  ...
  for (uint8_t idx = 0; idx < NUM_TAZOS; idx++) {
    for (uint8_t pixel = 0; pixel < LEDS_POR_TAZO; pixel++) {
      ...
      /*
        NOVA REGRA:
        guia overlay por cima da cena.
        Assim o fundo da musica continua ativo
        e a lane do input continua evidente.
      */
      if (guia_ativo[idx]) {
        cor_guia(idx, r, g, b);
      }

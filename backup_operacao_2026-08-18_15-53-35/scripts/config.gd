...
var _song_category_option: OptionButton
var _song_bpm_label: Label
...
var _song_detected_bpm: float = 0.0
var _bpm_detection_serial: int = 0
...
# ---------------------------------------------------------------
## Formulario de servico: nome, categoria, audio, video e capa. O BPM e
## detectado automaticamente. Os arquivos
...
func _build_song_form(screen: Vector2) -> void:
	...
	_song_name_edit.size = Vector2(width * 0.48, line * 0.86)
	_song_name_edit.placeholder_text = "EX: MINHA MÚSICA"
	_song_name_edit.max_length = 40
	_song_panel.add_child(_song_name_edit)

	# --- categoria ---
	var category_caption: Label = _make_label(
		"CATEGORIA",
		int(panel_size.y * 0.030),
		HORIZONTAL_ALIGNMENT_LEFT,
		font
	)
	category_caption.position = Vector2(margin + width * 0.51, y)
	category_caption.size = Vector2(width * 0.27, line * 0.52)
	category_caption.add_theme_color_override("font_color", Color(0.66, 0.74, 0.88, 1.0))
	_song_panel.add_child(category_caption)

	_song_category_option = OptionButton.new()
	_song_category_option.position = Vector2(margin + width * 0.51, y + line * 0.50)
	_song_category_option.size = Vector2(width * 0.27, line * 0.86)
	for category in USER_CATALOG.CATEGORIES:
		_song_category_option.add_item(category)
	_song_panel.add_child(_song_category_option)

	# BPM e somente leitura: nasce do arquivo de audio, nunca de digitacao.
	var bpm_caption: Label = _make_label(
		"BPM AUTOMÁTICO",
		int(panel_size.y * 0.026),
		HORIZONTAL_ALIGNMENT_LEFT,
		font
	)
	bpm_caption.position = Vector2(margin + width * 0.80, y)
	bpm_caption.size = Vector2(width * 0.20, line * 0.52)
	bpm_caption.add_theme_color_override("font_color", Color(0.66, 0.74, 0.88, 1.0))
	_song_panel.add_child(bpm_caption)

	_song_bpm_label = _make_label("AGUARDANDO MP3", int(panel_size.y * 0.030), HORIZONTAL_ALIGNMENT_CENTER, font)
	_song_bpm_label.position = Vector2(margin + width * 0.80, y + line * 0.50)
	_song_bpm_label.size = Vector2(width * 0.20, line * 0.86)
	_song_bpm_label.add_theme_color_override("font_color", Color(1.0, 0.82, 0.24, 1.0))
	_song_panel.add_child(_song_bpm_label)
...
func _on_audio_chosen(path: String) -> void:
	...
	_song_detected_bpm = 0.0
	_bpm_detection_serial += 1
	var detection_serial: int = _bpm_detection_serial
	...
	var embedded: float = USER_CATALOG.embedded_bpm(path)
	if embedded >= 40.0:
		_song_detected_bpm = embedded
		_refresh_song_form()
		return

	# Sem TBPM no MP3, escuta um trecho em uma via muda e mede a repeticao
	# dos ataques de energia. O operador nao informa numero algum.
	var analyzed: float = await _analyze_audio_bpm(path, detection_serial)
	if detection_serial != _bpm_detection_serial:
		return
	_song_detected_bpm = analyzed
	_refresh_song_form()
...
func _refresh_song_form() -> void:
	...
	if _song_bpm_label != null:
		if _song_audio_path.is_empty():
			_song_bpm_label.text = "AGUARDANDO MP3"
			_song_bpm_label.add_theme_color_override("font_color", Color(1.0, 0.82, 0.24, 1.0))
		elif _song_detected_bpm >= 40.0:
			_song_bpm_label.text = "%d BPM" % int(round(_song_detected_bpm))
			_song_bpm_label.add_theme_color_override("font_color", Color(0.32, 1.0, 0.62, 1.0))
		else:
			_song_bpm_label.text = "ANALISANDO..."
			_song_bpm_label.add_theme_color_override("font_color", Color(0.20, 0.86, 1.0, 1.0))
...
## Analisa o audio real sem enviar som aos alto-falantes. O efeito Capture
## recebe o PCM decodificado pelo proprio Godot e gera uma curva de energia
## a cada 10 ms; a autocorrelacao encontra o intervalo ritmico dominante.
func _analyze_audio_bpm(path: String, detection_serial: int) -> float:
	var stream: AudioStream = USER_CATALOG.load_audio(path)
	if stream == null:
		return 0.0

	var bus_index: int = AudioServer.bus_count
	var bus_name: String = "BPM_ANALYSIS_%d" % detection_serial
	AudioServer.add_bus(bus_index)
	AudioServer.set_bus_name(bus_index, bus_name)
	AudioServer.set_bus_volume_db(bus_index, -80.0)

	var capture := AudioEffectCapture.new()
	capture.buffer_length = 16.0
	...
	var mix_rate: int = maxi(8000, int(AudioServer.get_mix_rate()))
	var samples_per_bucket: int = maxi(1, int(float(mix_rate) * 0.01))
	var bucket_sum: float = 0.0
	var bucket_count: int = 0
	var analysis_seconds: float = minf(14.0, maxf(6.0, stream.get_length()))
	var steps: int = int(ceil(analysis_seconds / 0.10))

	if _song_status_label != null:
		_song_status_label.text = "Analisando o ritmo do áudio..."
		_song_status_label.add_theme_color_override("font_color", Color(0.20, 0.86, 1.0, 1.0))
	...
	for _step in range(steps):
		...
		for frame in frames:
			bucket_sum += (absf(frame.x) + absf(frame.y)) * 0.5
			bucket_count += 1
			if bucket_count >= samples_per_bucket:
				envelope.append(bucket_sum / float(bucket_count))
				bucket_sum = 0.0
				bucket_count = 0
	...
	if detection_serial != _bpm_detection_serial:
		return 0.0
	return _bpm_from_envelope(envelope, 100.0)


func _bpm_from_envelope(envelope: PackedFloat32Array, envelope_rate: float) -> float:
	if envelope.size() < 400:
		return 0.0

	var onset := PackedFloat32Array()
	onset.resize(envelope.size())
	var moving_average: float = 0.0
	var history: int = 8
	for index in range(envelope.size()):
		moving_average += envelope[index]
		if index >= history:
			moving_average -= envelope[index - history]
		var divisor: float = float(mini(index + 1, history))
		var local_average: float = moving_average / maxf(divisor, 1.0)
		onset[index] = maxf(0.0, envelope[index] - local_average * 1.08)

	var min_bpm: float = 60.0
	var max_bpm: float = 200.0
	var min_lag: int = maxi(1, int(round(envelope_rate * 60.0 / max_bpm)))
	var max_lag: int = mini(onset.size() - 1, int(round(envelope_rate * 60.0 / min_bpm)))
	var best_lag: int = 0
	var best_score: float = -1.0
	for lag in range(min_lag, max_lag + 1):
		var score: float = 0.0
		var energy: float = 0.0
		for index in range(lag, onset.size()):
			score += onset[index] * onset[index - lag]
			energy += onset[index] * onset[index]
		if energy > 0.000001:
			score /= energy
		var candidate_bpm: float = envelope_rate * 60.0 / float(lag)
		# Evita escolher metade do pulso quando a correlacao e praticamente igual.
		if candidate_bpm >= 82.0 and candidate_bpm <= 178.0:
			score *= 1.05
		if score > best_score:
			best_score = score
			best_lag = lag

	if best_lag <= 0 or best_score <= 0.01:
		return 0.0
	var detected: float = envelope_rate * 60.0 / float(best_lag)
	while detected < 70.0:
		detected *= 2.0
	while detected > 200.0:
		detected *= 0.5
	return clampf(round(detected), 40.0, 260.0)
...
func _save_new_song() -> void:
	var result: Dictionary = USER_CATALOG.add_song(
		...
		_song_video_path,
		_song_cover_path,
		_song_category_option.get_item_text(_song_category_option.selected),
		_song_detected_bpm
	)
